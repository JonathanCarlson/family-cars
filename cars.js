...
// toast) now lives in car-common.js, shared with Jordyn's jordyn.js. Only the
// Mach-E-specific ranking and rendering is below. Two copies of
...
const APP = createCarApp({
  ...
  shareText: 'The cars we\u2019re tracking \u2014 \u{1F44D}/\u{1F44E} the ones you like:',
  onReady: (data) => { DATA = data; render(); },
});
APP.boot();

// Aliases so the render code below reads unchanged.
const VOTES = APP.VOTES;
const COMMENTS = APP.COMMENTS;
const saveVotes = () => APP.saveVotes();
const setComment = (vin, t) => APP.setComment(vin, t);
const shareLink = () => APP.shareLink();
